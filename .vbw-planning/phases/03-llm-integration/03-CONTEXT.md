...
- Per-feature model configuration — different Claude models for different features:
  - Template Adapter (Phase 5): Sonnet 4.5 (`claude-sonnet-4-5-20250929`) — fast, cost-effective for Jinja2 placeholder insertion
  - Executive Report Generator (Phase 8) and any report look/section changes: Opus 4.6 (`claude-opus-4-6`) — highest quality for report generation
  - Default model for other/general use: Sonnet 4.5 (`claude-sonnet-4-5-20250929`)
  - Admin can override per-feature models in settings
  - LLM client accepts model parameter per request; calling features specify their model
