...
### Deviations
- DEV-01 / DEVN-01 — resolved-by-amendment. The test added a test-only withDbRetry retry/backoff wrapper around its DB writes (mirroring scheduleIsolation's upsertAssignmentWithRetry) not explicitly specified by the original task action. Classified plan-amendment (not a code defect) in R01-QA and resolved by amending 01-01-PLAN.md. The helper is correct, test-only, verified by TST-04, and stays unchanged; no product code modified.
