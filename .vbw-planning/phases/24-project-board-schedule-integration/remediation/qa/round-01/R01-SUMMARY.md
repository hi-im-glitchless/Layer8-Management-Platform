...
known_issue_outcomes:
  ...
  - '{"test":"scheduleIsolation.phase23 (concurrent run)","file":"backend/src/services/__tests__/scheduleIsolation.phase23.test.ts","error":"4/6 tests fail when run concurrently with scheduleIsolation.phase24.test.ts due to global-snapshot sensitivity in Phase 23 snapshotScheduleTables — Phase 23 still passes 6/6 when run in isolation","disposition":"accepted-process-exception","rationale":"Same root cause as the registry-shape duplicate above (different quoting of the apostrophe in Phase 23s vs Phase 23). Both entries reference the identical scheduleIsolation.phase23.test.ts:44-57 design flaw; both are accepted as a non-fixable-in-scope process exception for Phase 24 round 01. No code is changed; the Phase 23 maintenance refactor is a documented follow-up."}'
---
