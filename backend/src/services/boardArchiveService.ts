/**
 * boardArchiveService — card archive flow.
 *
 ...
 *   Project's `name` for the audit log). Any future edit must preserve
 ...
 *   link directly to Project, so the audit payload reads `card.project.name`.
 */
...
/**
 * Archive a board card. Hard-deletes attached files (DB rows + on-disk
 * bytes), and marks the card archived (`stage='archived'`,
 * `archivedAt=now()`). Comments + notes + checklist are preserved per
 * CONTEXT.md "metadata retention".
 *
 * The linked Project is read-only here — only its `name` is fetched for the
 * audit log, never written (Phase 24-R03 BoardCard→Project model).
 *
