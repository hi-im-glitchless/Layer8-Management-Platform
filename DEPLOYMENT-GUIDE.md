...
sudo systemctl restart layer8
```

If nginx config needs updating (e.g. after security changes to server blocks), also restore the full config:

```bash
cat << 'EOF' | sudo tee /etc/nginx/sites-available/layer8 > /dev/null
